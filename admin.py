...
def log_action(action, details="", target_user_id=None):
    ...
    try:
        ...
        if db_type == 'postgres':
            ...
            """)
            # SQLite "IF NOT EXISTS" for columns isn't widely available, so probe and add.
        ...
        if db_type == 'postgres':
            c.execute(
                "INSERT INTO audit_logs (admin_id, action, target_user_id, details, ip_address, timestamp) VALUES (%s, %s, %s, %s, %s, %s)",
                (admin_id, action, target_user_id, details, request.remote_addr, timestamp)
            )
        else:
            c.execute(
                "INSERT INTO audit_logs (admin_id, action, target_user_id, details, ip_address, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
                (admin_id, action, target_user_id, details, request.remote_addr, timestamp)
            )
        conn.commit()
...
def get_recent_activity():
    ...
    try:
        ...
        try:
            c.execute("SELECT id, admin_id, action, details, timestamp FROM audit_logs ORDER BY timestamp DESC LIMIT 10")
        except:
            c.execute("SELECT id, admin_id, action, details, timestamp FROM audit_logs ORDER BY timestamp DESC LIMIT 10")
        rows = c.fetchall()
        conn.close()

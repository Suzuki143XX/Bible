...
def get_comments():
    ...
    try:
        ...
        total = c.fetchone()[0]
        print(f"[DEBUG] Total comments in DB: {total}")
        
        # Simple query - get all non-deleted comments
        try:
            if db_type == 'postgres':
                c.execute("""
                    SELECT id, verse_id, text, timestamp, google_name, user_id, is_deleted
                    FROM comments
            ...
            rows = c.fetchall()
            print(f"[DEBUG] Found {len(rows)} non-deleted comments")
            
        except Exception as query_err:
            print(f"[ERROR] Query with is_deleted failed: {query_err}")
            ...
            if db_type == 'postgres':
                ...
                """)
            rows = c.fetchall()
            print(f"[DEBUG] Found {len(rows)} comments (fallback)")
        ...
        for row in rows:
            try:
                ...
                user_name = row[4] or "Anonymous"  # google_name
...
def get_audit_logs():
    try:
        ...
        try:
            ...
            # Fallback if columns still don't exist
            c.execute("SELECT id, admin_id, action, details, NULL as ip_address, timestamp FROM audit_logs ORDER BY timestamp DESC LIMIT 100")
        rows = c.fetchall()
        conn.close()

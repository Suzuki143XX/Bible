...
def log_action(action, details=""):
    try:
        ...
        if db_type == 'postgres':
            c.execute("INSERT INTO audit_logs (admin_id, action, details, ip_address, timestamp) VALUES (%s, %s, %s, %s, %s)",
    ...
    try:
        ...
        def get_count(query, params=None):
            ...
            if row is None:
                return 0
            try:
                # Try dict access first (PostgreSQL with RealDictCursor)
        ...
        try:
            if db_type == 'postgres':
                restricted = get_count("SELECT COUNT(*) as count FROM comment_restrictions WHERE expires_at > %s", (datetime.now().isoformat(),))
            else:
                restricted = get_count("SELECT COUNT(*) as count FROM comment_restrictions WHERE expires_at > ?", (datetime.now().isoformat(),))
        except:
            restricted = 0
        
        try:
            views = get_count("SELECT COALESCE(SUM(views), 0) as count FROM verses")
        except:
            views = 0
...
def get_comments():
    ...
    try:
        ...
        # Simple query without is_deleted filter first
        try:
            if db_type == 'postgres':
                c.execute("""
                    SELECT c.id, c.verse_id, c.text, c.timestamp, c.google_name, u.name, u.email
                    FROM comments c
            ...
            rows = c.fetchall()
            print(f"[DEBUG] Found {len(rows)} comments")
            
        except Exception as query_err:
            print(f"[ERROR] Query failed: {query_err}")
            # Try without JOIN
            if db_type == 'postgres':
                c.execute("""
                    SELECT id, verse_id, text, timestamp, google_name, user_id
                    FROM comments
                ...
                """)
            rows = c.fetchall()
            print(f"[DEBUG] Found {len(rows)} comments (without join)")
        ...
        for row in rows:
            try:
                comments.append({
                    ...
                    "email": row[6] if len(row) > 6 else "No email"
                })
            except Exception as row_err:
                print(f"[ERROR] Processing row: {row_err}, row={row}")
        
        print(f"[DEBUG] Returning {len(comments)} comments")

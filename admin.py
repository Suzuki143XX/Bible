...
def can_modify_role(admin_role, target_role, new_role):
    """Check if admin can modify target's role to new_role"""
    admin_level = get_role_level(admin_role)
    target_level = get_role_level(target_role)
    new_level = get_role_level(new_role)
    
    # Admin must be higher level than both target and new role
    if admin_level <= target_level:
        return False, "You cannot modify users at your level or higher"
    if admin_level <= new_level:
        return False, "You cannot assign roles at your level or higher"
    return True, None
...
def admin_login():
    """Admin login page - uses code-based access"""
...
def api_users():
    ...
    try:
        ...
        for row in c.fetchall():
            try:
                # Try dict-style access first (PostgreSQL)
                users.append({
                    'id': row['id'],
                    'email': row['email'],
                    'name': row['name'],
                ...
                # Fall back to index access (SQLite)
                users.append({
                    'id': row[0],
                    'email': row[2],
                    'name': row[3],
            ...
            if 'role' in data:
                ...
                if target:
                    try:
                        ...
                        target_role = target[0] or 'user'
                else:
                    target_role = 'user'
                
                # Check if admin can make this role change
                can_modify, error_msg = can_modify_role(admin_role, target_role, data['role'])
                if not can_modify:
                ...
                is_admin = 1 if data['role'] in ['owner', 'co_owner', 'mod', 'host'] else 0

...
def ensure_audit_logs_table(conn, c, db_type):
    """Create/migrate audit_logs so reads and writes do not fail on older schemas."""
    if db_type == 'postgres':
        c.execute("""
...
def log_action(action, details=""):
    try:
        conn, db_type = get_db()
        c = conn.cursor()
        ensure_audit_logs_table(conn, c, db_type)
        admin = get_admin_session()

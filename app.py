...
def get_stats():
    ...
    try:
        ...
        logger.info(f"Stats for user {session['user_id']}: verses={total}, liked={liked}, saved={saved}, comments={comments}, community={community}")
        
        # Return total comments including community for the profile count
        total_comments = comments + community
        
        return jsonify({"total_verses": total, "liked": liked, "saved": saved, "comments": total_comments})
    except Exception as e:
    ...
    try:
        ...
        for row in rows:
            ...
            if user_id:
                try:
                    if db_type == 'postgres':
                        c.execute("SELECT name, picture FROM users WHERE id = %s", (user_id,))
                    else:
                        c.execute("SELECT name, picture FROM users WHERE id = ?", (user_id,))
                    user_row = c.fetchone()
                    if user_row:
                        try:
...
def get_community_messages():
    ...
    try:
        ...
        if db_type == 'postgres':
            c.execute("""
                SELECT id, user_id, text, timestamp, google_name, google_picture
        ...
        for row in rows:
            ...
            # Use google_name as default
            user_name = google_name or "Anonymous"
            user_picture = google_picture or ""
